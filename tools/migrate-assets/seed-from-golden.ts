...
const SPACE_IDS = (process.env.SPACE_IDS ?? '285923')
  .split(',')
...
async function seedSpace(spaceId: number): Promise<void> {
  ...
  const metaFiles = allFiles.filter(f =>
    f.endsWith('.meta.json') && !path.basename(f).startsWith('_'),
  )
  ...
  const dbTasks = metaFiles.map(metaFile =>
    limiter.run(async () => {
      try {
        ...
        const shortFilename = asset.short_filename ??
          asset.filename?.split('/').pop()?.replace(/\.\w+$/, '') ?? ''
      ...
      process.stdout.write(
        `  DB [${done}/${metaFiles.length}] ✓${dbUpserted} ✗${dbFailed}\r`,
      )
    }),
  )
