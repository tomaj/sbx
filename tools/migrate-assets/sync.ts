...
async function syncSpace(spaceId: number): Promise<void> {
  ...
  if (deletedIds.length > 0) {
    console.log(
      `  Detected ${deletedIds.length} deleted assets — skipping removal (manual review recommended).`,
    );
    console.log(
      `  Deleted IDs: ${deletedIds.slice(0, 10).join(', ')}${deletedIds.length > 10 ? '…' : ''}`,
    );
  }
  ...
  const updatePromises = remoteAssets.map(async (asset) => {
    ...
    try {
      ...
      const needsUpdate = !localMeta || new Date(asset.updated_at) > new Date(localMeta.updated_at);
