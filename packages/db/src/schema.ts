...
export const spaces = pgTable('spaces', {
  ...
  languageCodes: json('language_codes').$type<string[]>().notNull().default([]),
  ...
  previewUrls: json('preview_urls').$type<Record<string, unknown>[]>().notNull().default([]),
  ...
  assetLibrarySettings: json('asset_library_settings')
...
export const users = pgTable('users', {
  ...
  favouriteSpaces: json('favourite_spaces').$type<number[]>().notNull().default([]),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
});
...
export const spaceMembers = pgTable(
  'space_members',
  {
    ...
    spaceRoleIds: json('space_role_ids').$type<number[]>().notNull().default([]),
...
export const apiTokens = pgTable(
  'api_tokens',
  {
    ...
    storyIds: json('story_ids').$type<number[]>().notNull().default([]),
    minCache: integer('min_cache').notNull().default(0),
    releaseIds: json('release_ids').$type<number[]>().notNull().default([]),
    createdAt: timestamp('created_at').notNull().defaultNow(),
  },
...
export const datasources = pgTable(
  'datasources',
  {
    ...
    dimensions: json('dimensions').$type<unknown[]>().notNull().default([]),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
...
export const components = pgTable(
  'components',
  {
    ...
    allPresets: json('all_presets').$type<Record<string, unknown>[]>().notNull().default([]),
    // internal_tags_list: [{id, name}]
    internalTagsList: json('internal_tags_list')
      .$type<Record<string, unknown>[]>()
      .notNull()
      .default([]),
    // internal_tag_ids: ["id1", "id2"]
    internalTagIds: json('internal_tag_ids').$type<string[]>().notNull().default([]),
...
export const spaceRoles = pgTable(
  'space_roles',
  {
    ...
    extId: text('ext_id'),
    permissions: json('permissions').$type<string[]>().notNull().default([]),
    allowedPaths: json('allowed_paths').$type<string[]>().notNull().default([]),
    blockedPaths: json('blocked_paths').$type<string[]>().notNull().default([]),
    fieldPermissions: json('field_permissions')
      .$type<Record<string, unknown>[]>()
      .notNull()
      .default([]),
    allowedFieldPermissions: json('allowed_field_permissions')
      .$type<Record<string, unknown>[]>()
      .notNull()
      .default([]),
    readonlyFieldPermissions: json('readonly_field_permissions')
      .$type<Record<string, unknown>[]>()
      .notNull()
      .default([]),
    datasourceIds: json('datasource_ids').$type<number[]>().notNull().default([]),
    blockedDatasourceIds: json('blocked_datasource_ids').$type<number[]>().notNull().default([]),
    componentIds: json('component_ids').$type<number[]>().notNull().default([]),
    allowedComponentIds: json('allowed_component_ids').$type<number[]>().notNull().default([]),
    branchIds: json('branch_ids').$type<number[]>().notNull().default([]),
    blockedBranchIds: json('blocked_branch_ids').$type<number[]>().notNull().default([]),
    allowedLanguages: json('allowed_languages').$type<string[]>().notNull().default([]),
    blockedLanguages: json('blocked_languages').$type<string[]>().notNull().default([]),
    assetFolderIds: json('asset_folder_ids').$type<number[]>().notNull().default([]),
    blockedAssetFolderIds: json('blocked_asset_folder_ids').$type<number[]>().notNull().default([]),
    managedComponentIds: json('managed_component_ids').$type<number[]>().notNull().default([]),
    blockedManageComponentIds: json('blocked_manage_component_ids')
      .$type<number[]>()
      .notNull()
      .default([]),
    managedComponentGroupUuids: json('managed_component_group_uuids')
      .$type<string[]>()
      .notNull()
      .default([]),
    blockedManageComponentGroupUuids: json('blocked_manage_component_group_uuids')
      .$type<string[]>()
      .notNull()
      .default([]),
    componentGroupUuids: json('component_group_uuids').$type<string[]>().notNull().default([]),
    blockedComponentGroupUuids: json('blocked_component_group_uuids')
      .$type<string[]>()
      .notNull()
      .default([]),
  },
...
export const webhookEndpoints = pgTable(
  'webhook_endpoints',
  {
    ...
    secret: text('secret'),
    actions: json('actions').$type<string[]>().notNull().default([]),
...
export const workflows = pgTable(
  'workflows',
  {
    ...
    spaceId: integer('space_id')
      ...
      .references(() => spaces.id, { onDelete: 'cascade' }),
    name: text('name').notNull(),
    contentTypes: json('content_types').$type<string[]>().notNull().default([]),
...
export const workflowStages = pgTable(
  'workflow_stages',
  {
    ...
    userIds: json('user_ids').$type<number[]>().notNull().default([]),
    spaceRoleIds: json('space_role_ids').$type<number[]>().notNull().default([]),
    workflowStageIds: json('workflow_stage_ids').$type<number[]>().notNull().default([]),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
  (t) => [
...
export const presets = pgTable(
  'presets',
  {
    ...
    name: text('name').notNull(),
    preset: json('preset').$type<Record<string, unknown>>().notNull().default({}),
    image: text('image'),
    color: text('color'),
    icon: text('icon'),
    description: text('description'),
...
export const activities = pgTable(
  'activities',
  {
    ...
    parameters: json('parameters').$type<Record<string, unknown>>().notNull().default({}),
    recipientId: bigint('recipient_id', { mode: 'bigint' }),
    recipientType: text('recipient_type'),
    // denormalized trackable/user info for display (mirrors Storyblok response)
    trackable: json('trackable').$type<Record<string, unknown>>().notNull().default({}),
    user: json('user').$type<Record<string, unknown>>().notNull().default({}),
...
export const stories = pgTable(
  'stories',
  {
    ...
    position: integer('position').notNull().default(0),
    tagList: json('tag_list').$type<string[]>().notNull().default([]),
    content: json('content').$type<Record<string, unknown>>().notNull().default({}),
    defaultFullSlug: text('default_full_slug'),
    translatedSlugs: json('translated_slugs').$type<Record<string, unknown>[] | null>(),
    ...
    releaseIds: json('release_ids').$type<number[]>().notNull().default([]),
    publishedData: json('published_data').$type<Record<string, unknown> | null>(),
    favouriteForUserIds: json('favourite_for_user_ids').$type<number[]>().notNull().default([]),
...
export const assets = pgTable(
  'assets',
  {
    ...
    metaData: json('meta_data').$type<Record<string, unknown>>().notNull().default({}),
...
export const datasourceEntries = pgTable(
  'datasource_entries',
  {
    ...
    dimensionValue: json('dimension_value').$type<Record<string, unknown>>().notNull().default({}),
    position: integer('position').notNull().default(0),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
...
export const releases = pgTable(
  'releases',
  {
    ...
    branchesToDeploy: json('branches_to_deploy').$type<number[]>().notNull().default([]),
    ownerId: bigint('owner_id', { mode: 'number' }),
    usersToNotifyIds: json('users_to_notify_ids').$type<number[]>().notNull().default([]),
    public: boolean('public').notNull().default(true),
    allowedUserIds: json('allowed_user_ids').$type<number[]>().notNull().default([]),
    allowedSpaceRoleIds: json('allowed_space_role_ids').$type<number[]>().notNull().default([]),
    allowedApiKeyIds: json('allowed_api_key_ids').$type<number[]>().notNull().default([]),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
...
export const tasks = pgTable(
  'tasks',
  {
    ...
    lastResponse: json('last_response').$type<Record<string, unknown> | null>(),
    webhookUrl: text('webhook_url'),
    userDialog: json('user_dialog').$type<Record<string, unknown>>().notNull().default({}),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
...
export const fieldTypes = pgTable('field_types', {
  ...
  spaceIds: json('space_ids').$type<number[]>().notNull().default([]),
...
export const comments = pgTable(
  'comments',
  {
    ...
    messageJson: json('message_json').$type<unknown[]>().notNull().default([]),
    uuid: text('uuid').notNull().unique(),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
  },
...
export const aiLogs = pgTable(
  ...
  (t) => [index('idx_ai_logs_space_created').on(t.spaceId, t.createdAt)],
);
