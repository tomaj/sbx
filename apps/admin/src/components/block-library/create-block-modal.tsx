...
export function CreateBlockModal({ open, groups, onConfirm, onCancel }: CreateBlockModalProps) {
  ...
  async function handleCreate() {
    if (!name.trim()) {
      setError('Technical name is required');
      return;
    }
    setSaving(true);
    setError(null);
  ...
  const _selectedGroup = groups.find((g) => g.uuid === groupUuid);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
        ...
        {/* Technical name */}
        <FormField label="Technical name" required description="Used in the JSON as block name">
          <input
            type="text"
            value={name}
