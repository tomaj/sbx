...
export function parseSchema(schema: Record<string, any>): {
  tabs: WorkingTab[];
  fields: WorkingField[];
} {
...
export function makeDefaultFieldDef(type: FieldType): AnyFieldDef {
  switch (type) {
    ...
    case 'boolean':
      return { type: 'boolean', default_value: false };
    case 'option':
      return { type: 'option', source: 'self', options: [] };
    case 'options':
      return { type: 'options', source: 'self', options: [] };
    case 'multilink':
      return { type: 'multilink', enable_advanced_search: true };
    case 'asset':
      return { type: 'asset' };
    case 'multiasset':
      return { type: 'multiasset' };
    case 'link':
      return { type: 'link' };
    case 'table':
      return { type: 'table' };
    case 'section':
      return { type: 'section', keys: [] };
    case 'bloks':
      return {
        type: 'bloks',
        component_whitelist: [],
