...
export function TagsView({ tags, isLoading, onCreateTag, onRenameTag, onDeleteTag }: TagsViewProps) {
  ...
  async function handleRename() {
    ...
    try {
      await onRenameTag(renameTag, renameName.trim())
      setRenameTag(null)
    } finally {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-4">
        {isLoading ? (
          <div className="space-y-2 mt-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="animate-pulse flex items-center gap-4 py-3 border-b border-gray-100 dark:border-gray-800">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="text-left py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Tag</th>
                <th className="text-left py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Blocks</th>
                <th className="py-3" />
              </tr>
            </thead>
            <tbody>
              {filtered.map((tag) => (
                <tr key={tag.id} className="border-b border-gray-100 dark:border-gray-800 group hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="py-3 pr-4 text-sm text-gray-600 dark:text-gray-400">{tag.count}</td>
                  <td className="py-3">
                    <div className="flex items-center gap-1 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => { setRenameTag(tag); setRenameName(tag.name) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setCreateOpen(false)} className="px-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg text-gray-700 dark:text-gray-300">Cancel</button>
              <button
      ...
      {renameTag && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={() => setRenameTag(null)} />
          <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') handleRename(); if (e.key === 'Escape') setRenameTag(null) }}
              className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 focus:outline-none focus:ring-1 focus:ring-teal-500 mb-4"
            />
            <div className="flex justify-end gap-3">
              <button onClick={() => setRenameTag(null)} className="px-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg text-gray-700 dark:text-gray-300">Cancel</button>
              <button
      ...
      <ConfirmModal
        ...
        confirmLabel="Delete"
        dangerous
        onConfirm={async () => { if (deleteTag) await onDeleteTag(deleteTag); setDeleteTag(null) }}
