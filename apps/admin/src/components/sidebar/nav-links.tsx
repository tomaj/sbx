...
import {
  LayoutGrid, Building2, ArrowLeft,
  Gauge, Monitor, Activity, Users, Code2, Settings,
  LayoutTemplate, Image, Database, Tag, ClipboardList,
} from 'lucide-react'
...
export function NavLinks() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/spaces')
      .then((r) => r.json())
      .then((data) => {
        const space = data.spaces?.find((s: { id: number }) => String(s.id) === spaceId)
    ...
    return (
      <>
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          <Link
            href="/spaces"
            className="flex items-center gap-3 px-3 py-2 rounded-md text-slate-400 hover:bg-white/10 hover:text-slate-200 transition-colors text-sm mb-2"
          >
