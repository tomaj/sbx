...
type Period = 'last_7_days' | 'this_month' | 'last_month' | 'last_3_months' | 'last_6_months' | 'last_12_months' | 'last_14_days'
...
function PeriodDropdown({ value, onChange, periods }: { value: Period; onChange: (p: Period) => void; periods: { value: Period; label: string }[] }) {
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
      ...
      <div className="flex items-center justify-between gap-4 mb-5">
        ...
        <div className="shrink-0">
          <PeriodDropdown value={period} onChange={tab === 'api_requests' ? setApiPeriod : setAiPeriod} periods={periods} />
        </div>
      </div>
          ...
          <table className="w-full text-xs text-left">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                ...
                {tab === 'ai_usage' && (
                  <th className="pb-2 font-medium text-gray-500 dark:text-gray-400 text-right">Tokens</th>
                )}
              </tr>
            </thead>
            <tbody>
              {chartData.map((d) => (
                <tr key={d.date} className="border-b border-gray-100 dark:border-gray-700/50 last:border-0">
                  ...
                  {tab === 'ai_usage' && (
                    <td className="py-1.5 text-right text-gray-500 dark:text-gray-400">{(d.tokens ?? 0).toLocaleString()}</td>
                  )}
                </tr>
              ))}
            </tbody>
