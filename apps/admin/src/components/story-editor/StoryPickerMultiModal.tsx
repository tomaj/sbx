...
export function StoryPickerMultiModal({
  spaceId,
  title,
  filterContentType,
  useUuid,
  ...
  const storiesUrl = `/api/admin/spaces/${spaceId}/stories?${params}`;

  const { data: storiesData, isLoading: loading } = useApi<{ stories: StoryItem[] }>(storiesUrl);
