...
import {
  Bold, Italic, Strikethrough, UnderlineIcon, Link as LinkIcon,
  List, ListOrdered, Minus, Undo, Redo, Eraser, Quote, Code, Code2,
  AlignLeft, AlignCenter, AlignRight, AlignJustify,
  Superscript as SuperscriptIcon, Subscript as SubscriptIcon,
  Highlighter, Palette, Image as ImageIcon, Table as TableIcon,
  Maximize2, Minimize2, Anchor,
} from 'lucide-react'
...
function ColorPicker({ label, colors, onSelect, activeColor, icon }: {
  label: string
  ...
  return (
    <div ref={ref} className="relative inline-flex">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg p-2 flex flex-wrap gap-1" style={{ minWidth: 120 }}>
          {colors.map((c) => (
...
export function RichtextField({ fieldKey, def, value, onChange }: Props) {
  ...
  const editor = useEditor({
    ...
    content: value && typeof value === 'object' && value.type === 'doc' ? value : { type: 'doc', content: [] },
    onUpdate: ({ editor }) => {
  ...
  useEffect(() => {
    if (!editor) return
    const incoming = value && typeof value === 'object' && value.type === 'doc' ? value : { type: 'doc', content: [] }
      ...
      <div className={wrapperClass}>
        ...
        <div className="flex flex-wrap items-center gap-0.5 px-2 py-1.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          ...
          <ColorPicker
            ...
            icon={<Palette className="w-4 h-4" style={activeTextColor ? { color: activeTextColor } : undefined} />}
          />
          ...
          <ColorPicker
            ...
            icon={<Highlighter className="w-4 h-4" style={activeHighlight ? { color: activeHighlight } : undefined} />}
          />
        ...
        {editor.isActive('table') && (
          <div className="flex flex-wrap items-center gap-0.5 px-2 py-1 border-b border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-blue-950/30 text-xs">
            ...
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().addColumnBefore().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">+ Col before</button>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().addColumnAfter().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">+ Col after</button>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().deleteColumn().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">− Col</button>
            <span className="text-blue-400 mx-0.5">|</span>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().addRowBefore().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">+ Row before</button>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().addRowAfter().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">+ Row after</button>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().deleteRow().run() }} className="px-1.5 py-0.5 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-700 dark:text-blue-300">− Row</button>
            <span className="text-blue-400 mx-0.5">|</span>
            <button type="button" onMouseDown={(e) => { e.preventDefault(); editor.chain().focus().deleteTable().run() }} className="px-1.5 py-0.5 rounded hover:bg-red-100 dark:hover:bg-red-900 text-red-600 dark:text-red-400">Delete table</button>
          </div>
        )}
