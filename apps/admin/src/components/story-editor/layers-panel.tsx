...
function getChildArrays(block: Record<string, any>): Array<{ fieldName: string; items: Record<string, any>[] }> {
...
function blockMatchesSearch(block: Record<string, any>, query: string): boolean {
  ...
  return getChildArrays(block).some(({ items }) => items.some(item => blockMatchesSearch(item, q)))
}
...
function FieldGroup({ fieldName, items, searchQuery, collapseKey }: FieldGroupProps) {
  ...
  const filteredItems = searchQuery ? items.filter(item => blockMatchesSearch(item, searchQuery)) : items
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-[10px] text-gray-500 dark:text-gray-400 font-medium">{formatName(fieldName)}</span>
      </button>
      {isExpanded && (
        <div className="ml-3">
          {filteredItems.map(item => (
            <LayerNode key={item._uid ?? item.component} block={item} searchQuery={searchQuery} collapseKey={collapseKey} />
          ))}
        </div>
      )}
    </div>
    ...
    <div>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs font-semibold text-gray-800 dark:text-gray-200 leading-tight">{name}</div>
          {subtitle && (
  ...
  return (
    <div className="w-72 flex flex-col border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden flex-shrink-0">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Components layers</h3>
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="p-2">
          <div className="flex items-center justify-between px-2 py-1.5 mb-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-teal-700 dark:text-teal-400">{rootName}</span>
            </div>
            {!searchQuery && (
              <button
                type="button"
          ...
          {filteredRootArrays.length === 0 && searchQuery && (
            <p className="text-xs text-gray-400 text-center py-4">No blocks match &ldquo;{searchQuery}&rdquo;</p>
          )}
        </div>
      </div>
    </div>
