...
function StatusBadge({ story }: { story: StoryDetail }) {
  if (story.published && !story.unpublished_changes) {
    return <span className="inline-flex items-center gap-1.5 text-teal-600 dark:text-teal-400"><span className="w-2 h-2 rounded-full bg-teal-500" />Published</span>
  }
  if (story.published && story.unpublished_changes) {
    return <span className="inline-flex items-center gap-1.5 text-amber-600 dark:text-amber-400"><span className="w-2 h-2 rounded-full bg-amber-500" />Published (unpublished changes)</span>
  }
  return <span className="inline-flex items-center gap-1.5 text-gray-500"><span className="w-2 h-2 rounded-full bg-gray-400" />Draft</span>
}

export function InfoTab({ story }: Props) {
  return (
    <div className="p-4 overflow-y-auto">
      <h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-3">Story Information</h3>
      <dl>
        <InfoRow label="Content type">
          {story.content_type ?? <span className="text-gray-400">—</span>}
        </InfoRow>
        <InfoRow label="Status">
          <div><StatusBadge story={story} /></div>
          {story.published_at && (
        ...
        <InfoRow label="Full slug">
          <code className="text-xs bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">{story.full_slug}</code>
        </InfoRow>
        <InfoRow label="UUID">
          <code className="text-xs bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">{story.uuid}</code>
        </InfoRow>
        <InfoRow label="ID">
          {story.id}
        </InfoRow>
          ...
          <InfoRow label={`Tags · ${story.tag_list.length}`}>
            <div className="flex flex-wrap gap-1 mt-1">
              {story.tag_list.map((tag) => (
                <span key={tag} className="px-2 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
