...
import { Plus, Settings, Trash2, ChevronDown, Lock, X, Check } from 'lucide-react';
...
function RoleForm({ spaceId, role, open, onClose, onSaved }: RoleFormProps) {
  ...
  const activeLangTags =
    form.languageMode === 'allowed' ? form.allowedLanguages : form.blockedLanguages;
                  ...
                  <span className="text-gray-600 dark:text-gray-400">
                    {form.pathMode === 'allowed'
                      ? form.allowedPaths.length === 0
                        ...
                        : `${form.allowedPaths.length} allowed path${form.allowedPaths.length !== 1 ? 's' : ''}`
                      : form.blockedPaths.length === 0
                        ? 'No blocked paths'
                        : `${form.blockedPaths.length} blocked path${form.blockedPaths.length !== 1 ? 's' : ''}`}
                  </span>
        ...
        {permTab === 'blocks' && (
          <div className="space-y-6">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <Toggle
                checked={form.grantBlockLibrary}
                onChange={(v) => set('grantBlockLibrary', v)}
              />
            </div>
        ...
        {permTab === 'datasources' && (
          <div>
            ...
            <PermissionSelector
              ...
              selectedIds={
                form.datasourceMode === 'allowed' ? form.datasourceIds : form.blockedDatasourceIds
              }
          ...
          <div className="space-y-4">
            <div>
              ...
              <PermissionSelector
                ...
                selectedIds={
                  form.assetMode === 'allowed' ? form.assetFolderIds : form.blockedAssetFolderIds
                }
                onSelectedChange={(ids) =>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={form.hideAssets}
                onChange={(e) => set('hideAssets', e.target.checked)}
                className="w-4 h-4 mt-0.5 rounded border-gray-300 text-teal-600 focus:ring-teal-500 shrink-0"
              />
              <span className="text-sm text-gray-700 dark:text-gray-300">
                Hide assets and assets folders (including their subfolders) the role doesn&apos;t
                have permission to upload to
              </span>
            </label>
          </div>
        )}
