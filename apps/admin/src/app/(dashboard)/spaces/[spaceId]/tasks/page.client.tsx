    ...
    >
      ...
      >
        <div className="space-y-1">
          ...
          <FormField
            label="Webhook"
            description={`Provide the endpoint where you want to send a POST request. The payload contains the task and space id. E.g.: {"task": {"id": 1, "name": "Sync job"}, "space_id": 12345}`}
          >
            <input
              type="url"
          ...
          <FormField
            label="User dialog"
            error={errors.user_dialog?.message}
            description="User dialog is a dialog which pops up when the user wants to execute the task."
          >
            <textarea
              {...register('user_dialog')}
              rows={8}
              className={`${inputCls} text-xs font-mono resize-none ${
                errors.user_dialog ? 'border-red-400' : ''
              }`}
            />
          </FormField>
